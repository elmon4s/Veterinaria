    ...
    public partial class FrmMascotaAtenciones : Form
    {
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            if (txtDescripcion.Text.Length < 1 && Formulario.Mensaje.Confirmacion("¿Seguro que quiere agregar una atencion SIN DESCRIPCIÓN?", "CUIDADO") == DialogResult.No)
                return;
            if (decimal.Parse(txtImporte.Text) <= 0 && Formulario.Mensaje.Confirmacion("¿Seguro que quiere agregar una atencion con IMPORTE IGUAL/INFERIOR A 0?", "CUIDADO") == DialogResult.No)
                return;
            Atencion a = new Atencion(0, txtDescripcion.Text, Convert.ToDecimal(txtImporte.Text), DateTime.Now);
            Atenciones.Add(a);
            dgvAtenciones.Rows.Add(new object[]
                {
                    a.Codigo.ToString(),
        ...
        {
            foreach (Atencion a in Atenciones)
            {
                if (a.Codigo == 0)
                    dbHelper.AgregarAtencion(codigo, a);
            }
            this.Close();
        }
